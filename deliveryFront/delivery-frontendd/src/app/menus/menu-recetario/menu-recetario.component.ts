...
@Component({
  ...
  templateUrl: './menu-recetario.component.html',
  styleUrl: './menu-recetario.component.scss'
})
export class MenuRecetarioComponent {
    ...
    this.productService.findAllProductsAvalible().subscribe({
      next: (response) => {
        if (response.success && response.data) {
          this.products = response.data.filter((product:ProductsDtoTable)=>product.type === 'RECETARIO')
